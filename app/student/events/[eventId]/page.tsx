...
export default async function StudentEventSignupPage({ params, searchParams }: PageProps) {
  ...
  const canceled = paramsData.canceled === "1";
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {existingTicket ? (
        <Card className="flex flex-col gap-3 border border-success/30 bg-success/10">
          ...
          <p className="text-sm text-success">
            Status: <span className="font-semibold">{existingTicket.checked_in_at ? "Sjekket inn" : existingTicket.status}</span>
          </p>
          ...
          <form action={cancelStudentEventTicket} className="mt-2">
            <input type="hidden" name="eventId" value={eventId} />
            <input type="hidden" name="returnTo" value={`/student/events/${eventId}`} />
            <Button type="submit" variant="danger" disabled={Boolean(existingTicket.checked_in_at)}>
              {existingTicket.checked_in_at ? "Kan ikke avbooke etter innsjekk" : "Avbook billett"}
            </Button>
          </form>
        </Card>
      ) : null}
